...
| [`colombod-building-an-audio-smart-tool.md`](findings/colombod-building-an-audio-smart-tool.md) | colombod | An audio mastering/cleanup tool built from nothing in a day -- [amplifier-smart-tools-audio](https://github.com/colombod/amplifier-smart-tools-audio). The licence wall: every mature audio library in Python is GPL-family, so "package a workflow as a tool" quietly meant reimplementing the domain. A contract between two agents is not a contract until something renders -- 11 divergences with a green suite, including a contract-conformant plan that crashed. **Never hand-write a mock**: three defect classes shipped past 284 passing tests because a fake built from the fields your own reader consults can only confirm what you already assumed; replaced with replays of real recordings, 330 tests and zero skips. A stale default model that no test can see. And an assertion that cannot fail -- the same chain 0.97 dB apart on two hosts, green on both. |
| [`colombod-guidance-and-evaluation-are-different-artifacts.md`](findings/colombod-guidance-and-evaluation-are-different-artifacts.md) | colombod | A controlled expertise probe of `aud` 0.11.1 — a shipped, 16/16-conformant tool that scored **0/24** on its own domain across eight induced spectral defects, while the *same model* through `aud`'s *own* prompt plus **one sentence of intent** scored 24/24, and a neutral nudge with **no API change** scored 15/20. Rendering confirms it at the audio: −0.01 to +0.38 dB of a 4–6.7 dB defect removed. A wrong-complaint control shows an intent parameter alone buys **obedience, not diagnosis** (15/15 compliance with a false complaint, +2.5 dB boosted into the real defect). The argument the measurement forced: in a dense domain the **evaluation belongs in a smart tool** (deterministic core, portable, produces a number) and the **guidance belongs in a bundle** (modes, staged context, cold critics — no CLI equivalent). Credits `amplifier-bundle-3d-developer` as the shape, and names the one thing that form lacks: a measurement behind its knowledge. Guidance half is a **PROPOSAL** — designed, not built. **Replicated at 0.12.0**: still **2/6** on induced spectral defects two releases later, while the *same model asked outside the tool* maps the same vocabulary **6/6** — localising the gap to the interface, not the model, by construction. Plus a `bad_model_output` failure on the clean control (1 of 7 runs), and a warning that cost the first result: a grader built from guessed field names scored a confident, publishable **6/6 MISS** that was entirely false |
| [`colombod-auditing-smart-tools.md`](findings/colombod-auditing-smart-tools.md) | colombod | Auditing the first four smart tools: **Round 1** puts `aud` 0.10.0 and `vid` 0.3.1 through the full ladder. Both passed 16/16 conformance while carrying 10 spec deviations each; `check-spec-adherence` is a sampling process (three runs named real new instances). The three worst defects passed all checks: `vid zoom` 50× duration bug, 10 of 12 composed audio chains broken, scene detection persisting failures as success. Plus 1024-char skill description limit breached by 3 of 4 tools and checked by nothing. **Round 2** applies the same ladder to `deep-research` and `fact-check`: 6 adhere / 10 deviate each, then a real-spend output correctness sweep. Found 8 defects including 404 citations, `fact-check verdicts` always null (fixture never hand-written again), and cost under-reporting 13.8×. Positive result: deep-research → fact-check composition works. Plus rung 6 of the ladder: run what CI runs before you push. |
| [`colombod-a-smart-tool-that-measures-its-own-staleness.md`](findings/colombod-a-smart-tool-that-measures-its-own-staleness.md) | colombod | Replacing an MCP bundle with a smart tool, [`lore`](https://github.com/colombod/amplifier-smart-tools-lore), for library and project knowledge. Two properties of a knowledge domain break the spec in specific places: **its answers have an age, and its payloads do not fit.** One conformant `read_wiki_contents` response is **2,412,495 bytes carrying 1,169,101 characters across 130 pages, ~292,000 tokens** - and the bundle it replaces documented that as "truncated to ~50k chars", which is simply false. Nothing in the spec or the kit can see response size. DeepWiki publishes its indexed commit **in the page** (`23843e9c`, 20 July 2026), so staleness is measurable against the GitHub compare API rather than asked of a model as the bundle did: **82, then 83, then 84 commits behind in two hours**. Both defects that mattered were found by running, not testing, after 164 green tests and a clean model review: a continuation string the CLI rejected, and caller-supplied material that borrowed an unrelated repository's staleness as its provenance. `check-spec-adherence` sampled **6 -> 2 -> 4** across three runs with no overlap, now **10 runs over 5 tools, never clean**, and its headline line printed a *positive* first evidence item on 3 of 4 real deviations. Adds live evidence to `proposals/output-is-bigger-than-the-response.md`. Argues the spec's "payload is data, not a reference" costs a knowledge tool its provenance, and that **"a missing prerequisite fails immediately" is not satisfiable** where the SDK exposes no entitlement check. One prompt through Claude Code, Codex and Amplifier: all three carried the measured age into their own answers, and `--json` did more of that work than the skill |
...
| [`proposals/the-kit-should-check-the-skill-it-tells-you-to-ship.md`](proposals/the-kit-should-check-the-skill-it-tells-you-to-ship.md) | the spec (conformance kit) | The 1024-character Agent Skills description limit is not checked by the conformance kit, breached by 3 of 4 tools, and degrades silently in the host. Proposes one deterministic rule: parse `skills/*/SKILL.md` frontmatter and fail if description exceeds 1024 characters |
