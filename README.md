...
| [`colombod-auditing-smart-tools.md`](findings/colombod-auditing-smart-tools.md) | colombod | Putting two already-shipped, already-conformant tools (`aud` 0.10.0, `vid` 0.3.1) through `smart-tool-creator`'s two checks, then through something neither check does: rendering the output and measuring it. **Both scored 16/16 on the conformance kit while carrying 10 spec deviations each.** `check-spec-adherence` is a sampling process, not an enumeration -- three runs on one tool, each naming real new instances of the same classes. And the three worst defects passed every check available: `vid zoom` rendered **400 seconds from an 8-second source** and silently dropped 640x360 to 320x180 with 175 green tests and a model reviewer that had read the source; **10 of 12 composed audio chains were broken** while every verb worked alone; and a failed scene detection persisted an index recording failure as success. Plus the Agent Skills **1024-character description limit that nothing checks** -- breached by 3 of our 4 skills, degrading silently in the host where no author is watching. |
